 ...
 * @version: 
 * @Author: CC
 * @Date: 2021-02-12 14:55:05
 * @LastEditors: sueRimn
 * @LastEditTime: 2021-02-12 17:51:09
 */
//Node中的事件循环模型
/*
...
/*
  ...
  第四个阶段：poll(轮询阶段，核心)
            ---如果回调队列里有待执行的回调函数
                从回调队列中取出回调函数，同步执行(一个一个执行)，直到回调队列为空了，或者达到系统最大限度。(不会执行定时器的回调，定时器的回调之只能第一阶段执行)
            ---如果回调队列为空
                  ---如果有设置过setImmediate
